...
| **Multi-instrument → solo-piano reduction — Stage 1: source separation** — an `ISourceSeparator` port + a **Spleeter 5-stem** ONNX adapter + a `separate` verb, guarded by a synthesize→separate **SI-SDR** regression gate (a statistical floor against pipeline regressions — never a quality *proof*; see `docs/CORPUS.md` "Corpus 3"). The falsifiable half (Stages 1–3 → multi-track MIDI); arrangement (Stage 4) is a separate, later, opt-in verb. | [design](2026-07-13-multi-instrument-piano-reduction-design.md) | [Stage-1 plan](2026-07-14-stage1-source-separation-plan.md) | ✅ **Stage 1 DONE** (branch `stage-1-source-separation`, 747/747 tests green). All of 1.0–1.5 shipped: the 5-stem Spleeter ONNX export (native arm64 — no Docker; per-branch fp32 logit ONNX, softmax lifted to C#; independently-verified 6.3e-4 ONNX↔TF parity), the STFT front end + iSTFT (golden-validated 3.6e-6 rel / round-trip 6e-14), the `SpleeterSourceSeparator` adapter, the SI-SDR closed-loop gate (median 12.1 dB, gate 6.0), and the `separate` verb. Model = Spleeter 5-stem (only piano-stem model; strict §1.7 survey found no clean pass — [`DECISIONS.md`](../../DECISIONS.md) "Source separation"). **Open human gate:** does a separated piano stem from a real, rights-cleared recording sound structurally intact (R11.2-style, Cornelius). **Stages 2+ DONE** (branch `stage-1-source-separation`, Fast 728/728 + new [Slow] tests green) in [`2026-07-15-stage2plus-multi-instrument-piano.md`](2026-07-15-stage2plus-multi-instrument-piano.md): the goal was refined (Cornelius) to **"all notes on piano"** — per-stem transcription (Transkun on piano, Basic Pitch on the rest; drums dropped, vocals `--include-vocals`) → a faithful **multi-track MIDI** + a combined grand-staff piano `score.mid`/`score.musicxml` + a piano `recreation.wav`. Shipped as the batch **`claudio pianize <mix.wav>`** verb (Stage 2 `MultiStemTranscriber`/`MultiStemRouting` + Stage 3 `MultiTrackMidiWriter`) and the **`listen --separate`** live prototype (bounded ~7 s window tick + full-quality capture-then-pianize on Stop; gated by the `SeparationLiveSpike` measurement that proved the naive whole-buffer path degrades). This is NOT the deferred Stage-4 *playable reduction* — it's the faithful transcription collapsed onto piano (dense, statistical, never "proven"). **Open human gate:** does a real recording's piano `recreation.wav` render a recognizable tune. See [`DECISIONS.md`](../../DECISIONS.md) "Multi-instrument → piano" + "Live-separated `listen --separate` prototype". |
