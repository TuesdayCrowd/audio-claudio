...
Code MIT (`LICENSE.spleeter`, Deezer SA). Weight grant is **ambiguous** (repo README says "the *code* is MIT"; the JOSS paper, DOI 10.21105/joss.02154, says "source code *and pre-trained models* are… distributed under a MIT license"; a clarification request has been open + unanswered since 2024). Training data is Deezer's **private "Bean" catalog — not MUSDB** — so there is no third-party non-commercial encumbrance; the only doubt is Deezer's own grant wording, which their published peer-reviewed paper resolves in our favor. That JOSS models-grant (plus the good-faith clarification request on record, `deezer/spleeter#898`) is the load-bearing basis for committing the weights, with `LICENSE.spleeter` alongside and the ambiguity disclosed here in the open — **not** this project's non-commercial nature (which would bound the *stakes* of the pessimistic reading, not answer the copyright question: unauthorized redistribution is barred regardless of the copier's ultimate use). Not to be re-cited as unconditionally "MIT."
